defmodule Aoc.Day01 do
  ...
  def solve_part_02 do
    load_input() |> find_duplicate()
  end

  @doc """
  Compute a stream.

      iex> Aoc.Day01.compute("+1 +1 +1")
      3

      iex> Aoc.Day01.compute("+1 +1 -2")
      0

      iex> Aoc.Day01.compute("-1 -2 -3")
      -6
  """
  def compute(input) do
    ...
    |> String.trim()
    |> String.split(~r/\s/)
    |> Enum.reduce(0, fn val, acc -> acc + parse_value(val) end)
  end

  @doc """
  Find the first duplicate in a stream.

      iex> Aoc.Day01.find_duplicate("+1 -1")
      0

      iex> Aoc.Day01.find_duplicate("+3 +3 +4 -2 -4")
      10

      iex> Aoc.Day01.find_duplicate("-6 +3 +8 +5 -6")
      5

      iex> Aoc.Day01.find_duplicate("+7 +7 -2 -7 -4")
      14
  """
  def find_duplicate(input) do
    ...
    |> String.split(~r/\s/)
    |> Stream.cycle()
    |> Enum.reduce_while({0, MapSet.new([0])}, fn val, {acc, set} ->
      acc = acc + parse_value(val)

      if MapSet.member?(set, acc) do
        {:halt, acc}
      else
        {:cont, {acc, MapSet.put(set, acc)}}
      end
    end)
  end
